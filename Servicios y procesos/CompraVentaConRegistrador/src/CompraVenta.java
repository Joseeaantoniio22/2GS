 ...
 * que acceden concurrentemente al mismo. Antes de esto, lanza un proceso
 * servidor que recibirá y guardará los mensajes generados y enviados por los
 * procesos anteriores.
 * 
...
public class CompraVenta {
  ...
  public static void main(String[] args) {
    ...
    // Creamos el proceso Registrador que actuará como servidor de mensajes
    try {

      //El proceso se lanzará con el comando: "cmd /c start cmd /K \"java ... \""
      String cmd = "cmd /c start cmd /K \"java Registrador.java\"";
      //Para lanzar los procesos usamos el método Runtime.getRuntime.exec(...)
      String[] separador = cmd.split(" ");
      Runtime.getRuntime().exec(separador);
      //Imprime por consola un mensaje indicando que se ha creado el proceso registrador
      System.out.println("Proceso registrador creado");


    } catch (Exception ex) {
      System.err.println("CompraVenta: No se pudo crear el proceso registrador.");
    }

    // Creamos un grupo de procesos jugadores que accederán al mismo inventario
    try {      

      // Crear un bucle para lanzar los procesos jugadores (tantas veces como indique NUMERO_JUGADORES)
      // Cada proceso se lanzará con el comando: "cmd /c start cmd /K \"java Jugador.java ... ... \""
      // Para lanzar los procesos usamos el método Runtime.getRuntime.exec(...)
      // Imprime por consola un mensaje indicando que se ha creado el proceso jugador (y su número)

      for (int i = 0; i < NUMERO_JUGADORES; i++) {
        String cmd = "cmd /c start cmd /K \"java Jugador.java "+i+" ../inventario.txt \"";
        String[] separador = cmd.split(" ");
        Runtime.getRuntime().exec(separador);
        System.out.println("Proceso creado del jugador: "+(i+1));
      }
      
    } catch (Exception ex) {
      System.err.println("CompraVenta: No se pudo crear el proceso jugador.");
    }
  }

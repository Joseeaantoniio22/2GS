    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < NUMERO_OPERACIONES; i++) {
            ...
            try {
                ...
                // TODO: Bloquea el archivo con el inventario para evitar acceso concurrente no sincronizado
                // TODO: Elige aleatoriamente qué acción realizará el jugador (comprar o vender un artículo)
                // TODO: Elige aleatoriamente el ID del artículo sobre el que realizará la acción (1, 2 ó 3)
                // TODO: Realiza la acción correspondiente (compra o venta) y guarda el mensaje resultante                
                // TODO: Imprime el mensaje sobre la transacción por consola
                // TODO: Libera el bloqueo del archivo con el inventario
                // TODO: Cierra el fichero con el inventario
                FileLock block = null;
                block = inventario.getChannel().lock();
                Random accion = new Random();
                int accionAleatorio = accion.nextInt(1 - 0 + 1) + 0;
                Random articulo = new Random();
                int idArticulo = articulo.nextInt(3 - 1 + 1) + 1;
                if (accionAleatorio == 0) {
                    venderArtículo(inventario, idArticulo);
                    System.out.println("Se ha vendido el articulo con ID " + idArticulo);
                } else {
                    comprarArtículo(inventario, idArticulo);
                    System.out.println("Se ha comprado el articulo con ID " + idArticulo);
                }
                block.release();
                inventario.close();
